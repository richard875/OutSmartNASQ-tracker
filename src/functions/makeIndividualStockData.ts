...
const makeIndividualStockData = async (stock: any) => {
  ...
  const priceChanged: number = stockRaw.c - stockRaw.pc;
  const percentageChanged: number =
    ((stockRaw.c - stockRaw.pc) / stockRaw.pc) * 100;
  text += "\n<tr>";
  text += `\n<td>${stockInfo[0].InstrumentDisplayName}</td>`;
  text += `\n<td><a href='https://finance.yahoo.com/quote/${stockInfo[0].SymbolFull}'>${stockInfo[0].SymbolFull}</a></td>`;
  text +=
    percentageChanged >= 0
  ...
  text +=
    percentageChanged >= 0
      ? `\n<td><span style='color: green;'>${priceChanged.toFixed(
          2
        )} | ${percentageChanged.toFixed(2)}%</span></td>`
      : `\n<td><span style='color: red;'>${priceChanged.toFixed(
          2
        )} | ${percentageChanged.toFixed(2)}%</span></td>`;
